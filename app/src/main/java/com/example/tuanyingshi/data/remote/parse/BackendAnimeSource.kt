...
/**
 * 后端模式数据源：把自建后端 `/api/v1` 内容接口适配成 [AnimeSource]。
 *
 * 仅在 [com.example.tuanyingshi.util.BackendPrefs.isBackendMode] 为 true 时，
 * 由 [com.example.tuanyingshi.util.SourceHolder] 的三个 getter 返回，
 * 对当前的规则 / CSS 数据源模式**零侵入**——下游各 ViewModel 与 Repository 完全无感。
 */
object BackendAnimeSource : AnimeSource {

    private const val TAG = "BackendAnimeSource"
    private const val SOURCE_NAME = "团影视后端"

    override val DEFAULT_DOMAIN: String get() = "backend"
    override var baseUrl: String = ""
    override var WEB_URL: String = "backend"
    ...
    private fun AnimeCardVO.toBean(): AnimeBean {
        // AnimeBean.url 即详情地址（toAnime() 会映射到 Anime.detailUrl），后端模式下用站内 id 作为该 key。
